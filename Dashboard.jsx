    ...
    <div onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      {onReport && hov && (
        <button onClick={e => { e.stopPropagation(); onReport(reportHint); }}
          style={{ position:"absolute", bottom:10, right:12, background:"none", border:"none", cursor:"pointer", fontSize:10, color:BRAND.purple, fontWeight:600, fontFamily:"inherit" }}>
          View report ↗
        </button>
      )}
    </div>
...
function Card({ children, style }) {
  return (
    <div style={{ background: BRAND.cardBg, borderRadius: 14, padding: "20px 22px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)", ...style }}>
      {children}
    </div>
  );
}
...
export default function Dashboard({ environment, session, onNavigate, onOpenRecord, onReport }) {
  ...
  const load = useCallback(async (force = false) => {
    ...
    try {
      ...
      const objectData = objects.map((o, i) => {
        const res = recordResults[i]; const records = Array.isArray(res?.records) ? res.records : [];
      ...
      const buildMonthly = (obj) => months.map(m => ({
        ...m, count: (obj?.records || []).filter(r => { const d = new Date(r.created_at||0); return d.getMonth()===m.month&&d.getFullYear()===m.year; }).length,
      }));
  ...
  const goTo = (objectSlug, filterKey, filterValue) =>
  ...
  return (
    <div style={{ background:BRAND.pageBg, minHeight:"100vh", padding:"28px 32px", fontFamily:"'DM Sans',-apple-system,sans-serif" }}>
      ...
      <div style={{ display:"flex", gap:6, marginBottom:20, flexWrap:"wrap" }}>
        {depts.map(d => (
          <button key={d} onClick={() => setDeptFilter(d)}
            style={{ fontSize:11, padding:"4px 12px", borderRadius:20, border:"none", background: deptFilter===d ? BRAND.purple : BRAND.cardBg, color: deptFilter===d ? "#fff" : BRAND.gray, cursor:"pointer", fontFamily:"inherit", fontWeight: deptFilter===d ? 700 : 400, boxShadow: deptFilter===d ? "none" : "0 1px 3px rgba(0,0,0,0.05)", transition:"all 0.15s" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,minmax(0,1fr))", gap:12, marginBottom:16 }}>
        <KpiCard label="Active candidates" value={activeCandidates.toLocaleString()} sub="vs last month"
          tag={momPeople!==null?(momPeople>=0?`+${momPeople}%`:`${momPeople}%`):null} tagKind={momPeople>=0?"up":"down"} color={BRAND.purple}
          onClick={() => goTo("people","status","Active")} onReport={openReport}
          reportHint={{ object:"people", title:"Candidates by status", groupBy:"status", chartType:"bar" }} />
        <KpiCard label="Open roles" value={openRoles.toLocaleString()} sub="active requisitions"
          tag={momJobs!==null?(momJobs>=0?`+${momJobs}%`:`${momJobs}%`):null} tagKind={momJobs>=0?"up":"neutral"} color={BRAND.rose}
          onClick={() => goTo("jobs","status","Open")} onReport={openReport}
          reportHint={{ object:"jobs", title:"Jobs by department", groupBy:"department", chartType:"bar" }} />
        <KpiCard label="Talent pools" value={poolCount.toLocaleString()} sub="active pools" color={BRAND.teal}
          onClick={() => goTo("talent-pools","","")} onReport={openReport}
          reportHint={{ object:"talent-pools", title:"Pools by category", groupBy:"category", chartType:"pie" }} />
      ...
      <div style={{ display:"grid", gridTemplateColumns:"minmax(0,1.75fr) minmax(0,1fr)", gap:12, marginBottom:16 }}>
        ...
        <Card>
          <CardHeader title="Pipeline velocity" sub="Candidates added each month"
            action={<button onClick={() => openReport({ object:"people", title:"Monthly pipeline", chartType:"area" })} style={{ fontSize:10, color:BRAND.purple, background:"none", border:"none", cursor:"pointer", fontFamily:"inherit", fontWeight:700 }}>Open report ↗</button>} />
          ...
          <div style={{ display:"flex", gap:16, marginTop:12 }}>
            <LegendItem color={BRAND.purple} label="Candidates added" onClick={() => goTo("people","","")} />
          </div>
        </Card>
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={160}>
            <PieChart>
              <Pie
                data={data?.sourceBreakdown?.length ? data.sourceBreakdown : [{ name:"Direct",value:40 },{ name:"Referral",value:28 },{ name:"Agency",value:18 },{ name:"LinkedIn",value:14 }]}
                cx="50%" cy="50%" innerRadius="62%" outerRadius="82%" dataKey="value" paddingAngle={3}
                onClick={handlePieClick} style={{ cursor:"pointer" }}>
                {(data?.sourceBreakdown?.length ? data.sourceBreakdown : [{name:"Direct"},{name:"Referral"},{name:"Agency"},{name:"LinkedIn"}]).map((_, i) => (
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:"4px 12px", marginTop:8 }}>
            {(data?.sourceBreakdown?.length ? data.sourceBreakdown : [{name:"Direct"},{name:"Referral"},{name:"Agency"},{name:"LinkedIn"}]).map((s, i) => (
              <LegendItem key={i} color={ACCENT_COLORS[i%ACCENT_COLORS.length]} label={s.name} onClick={() => goTo("people","source",s.name)} />
            ))}
          </div>
        </Card>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"minmax(0,1.3fr) minmax(0,1fr)", gap:12 }}>
        ...
        <Card>
          <CardHeader title="Roles by department" sub="Click a bar to filter those roles"
            action={<button onClick={() => openReport({ object:"jobs", title:"Jobs by department", groupBy:"department", chartType:"bar" })} style={{ fontSize:10, color:BRAND.purple, background:"none", border:"none", cursor:"pointer", fontFamily:"inherit", fontWeight:700 }}>Open report ↗</button>} />
          <ResponsiveContainer width="100%" height={220}>
            <BarChart
              data={data?.deptBreakdown?.length ? data.deptBreakdown : [{ dept:"Engineering",open:12,filled:9 },{ dept:"Sales",open:8,filled:6 },{ dept:"Finance",open:5,filled:4 },{ dept:"Marketing",open:6,filled:5 },{ dept:"HR",open:3,filled:2 }]}
              margin={{ top:4, right:4, bottom:0, left:-20 }} barCategoryGap="30%"
        ...
        <Card>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:2, marginTop:4 }}>
            {funnelValues.map((s, i) => (
              <FunnelRow key={i} label={s.label} value={s.value} pct={Math.round((s.value/maxFunnel)*100)} color={s.color} onClick={() => goTo("people","status",s.label)} />
            ))}
          </div>
        ...
        <Card style={{ marginTop:16 }}>
          ...
          <div>
            {data.activity.slice(0,6).map((a, i) => (
              <ActivityRow key={i} a={a} isLast={i===5} onOpenRecord={onOpenRecord} />
            ))}
          </div>
        </Card>
      )}
...
// Separate component to avoid hooks-in-map lint issue
function ActivityRow({ a, isLast, onOpenRecord }) {
  const [hov, setHov] = useState(false);
  const canClick = !!(a.record_id && onOpenRecord);
  return (
    <div
      onClick={() => canClick && onOpenRecord(a.record_id, a.object_id)}
      ...
      <div style={{ fontSize:11, color:"#888780", flexShrink:0 }}>{a.created_at ? new Date(a.created_at).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}) : "—"}</div>
