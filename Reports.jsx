...
function evalFormula(expr, row) {
  try {
    ...
    const nv = (k) => { const v=row[k]; return typeof v==="number"?v:parseFloat(v)||0; };
...
function Panel({ title, sub, children, action }) {
  return (
    <div style={{ background:B.card, borderRadius:14, padding:"20px 22px", boxShadow:"0 1px 4px rgba(0,0,0,0.04)" }}>
...
function Pill({ label, active, onClick }) {
  return (
    <button onClick={onClick} style={{ fontSize:11, padding:"4px 12px", borderRadius:20, cursor:"pointer", border:"none", background: active ? B.purple : B.card, color: active ? "#fff" : B.gray, fontFamily:"inherit", fontWeight: active ? 700 : 400, boxShadow: active ? "none" : "0 1px 3px rgba(0,0,0,0.06)", transition:"all 0.15s" }}>
      {label}
    </button>
  );
}
...
// ── Field inputs ──────────────────────────────────────────────────────────
const OPS = {
  text:["contains","is","is not","is empty","is not empty"], number:["=","≠",">","<","≥","≤","is empty"],
  select:["is","is not","is empty"], multi_select:["includes","does not include","is empty"],
  date:["is","before","after","is empty"], boolean:["is true","is false"],
  rating:["=",">","<"], currency:["=",">","<","≥","≤"],
};

const Inp = ({ val, onChange, placeholder, style }) => (
  <input value={val} onChange={e=>onChange(e.target.value)} placeholder={placeholder}
    style={{ fontSize:12, padding:"6px 10px", borderRadius:8, border:`0.5px solid rgba(0,0,0,0.1)`, background:"white", color:"#111827", fontFamily:"inherit", width:"100%", outline:"none", ...style }} />
);

const Sel = ({ val, onChange, opts }) => (
  <select value={val} onChange={e=>onChange(e.target.value)}
    style={{ fontSize:12, padding:"6px 10px", borderRadius:8, border:`0.5px solid rgba(0,0,0,0.1)`, background:"white", color:"#111827", fontFamily:"inherit", width:"100%", outline:"none" }}>
...
const SideLabel = ({ children }) => (
  <div style={{ fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.08em", color:B.gray, marginBottom:6 }}>{children}</div>
);
...
export default function Reports({ environment, initialReport }) {
  ...
  useEffect(() => {
    ...
    api.get(`/api/saved-views?environment_id=${environment.id}`).then(d => setSavedList(Array.isArray(d)?d:[]));
  }, [environment?.id]);
  ...
  useEffect(() => {
    ...
    api.get(`/api/fields?object_id=${selObject}`).then(d => {
      ...
      if (!skipReset.current) {
        ...
        setGroupBy(""); setSortBy(""); setFilters([]); setFormulas([]);
        setChartX(""); setChartY(""); setResults(null); setActiveChartFilter(null);
      }
  ...
  const runReport = useCallback(async (objectId, grpBy) => {
    ...
    try {
      ...
      if (af.length) rows = rows.filter(row => af.every(f => {
      ...
      if (gb) {
        const groups = {};
        rows.forEach(r => { const k=String(r[gb]||"Unknown"); if(!groups[k])groups[k]=[]; groups[k].push(r); });
        grouped = Object.entries(groups).map(([key,members]) => ({
          _group:key, _count:members.length,
          ...members.reduce((acc,m) => { Object.keys(m).forEach(k=>{ if(typeof m[k]==="number")acc[k]=(acc[k]||0)+m[k]; }); return acc; }, {}),
        }));
      }
  ...
  const saveReport = async () => {
    ...
    if (d?.id) { setSavedList(p=>[...p,d]); setListName(""); setShowSaveForm(false); }
    setSavingList(false);
  };
  ...
  // Chart click → filter results table to that group
  const handleChartClick = (data) => {
    if (!data?.activePayload?.[0]) return;
    const clickedGroup = data.activePayload[0].payload?.[chartX] ?? data.activePayload[0].payload?._group;
    if (!clickedGroup) return;
    setActiveChartFilter(prev => prev === String(clickedGroup) ? null : String(clickedGroup));
  };
  const handlePieClick = (entry) => {
    const val = entry?.name || entry?.[chartX];
    if (!val) return;
    setActiveChartFilter(prev => prev === String(val) ? null : String(val));
  };

  const displayedResults = activeChartFilter && results
    ? results.filter(r => String(r[chartX]||r._group||"") === activeChartFilter)
    : results;
  ...
  // Navigate to filtered record list
  const goToFiltered = (filterKey, filterValue) => {
    ...
    window.dispatchEvent(new CustomEvent("talentos:filter-navigate", {
      detail: { objectSlug: obj.slug, fieldKey: filterKey, fieldValue: filterValue },
    }));
  };

  const exportCSV = () => {
    if (!displayedResults) return;
    const cols = fields.filter(f => selCols.includes(f.id));
    const headers = cols.map(f=>f.name).join(",");
    const rows = displayedResults.map(r => cols.map(f => JSON.stringify(r[f.api_key]??"")));
    ...
    <div style={{ background:B.bg, minHeight:"100vh", padding:"28px 32px", fontFamily:"'DM Sans',-apple-system,sans-serif" }}>
      ...
      <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between", marginBottom:24 }}>
        ...
        <div style={{ display:"flex", gap:8 }}>
          <button onClick={() => { setShowSaveForm(true); setPanel("saved"); }}
            style={{ fontSize:11, padding:"6px 14px", borderRadius:20, border:"none", background:B.card, color:B.gray, cursor:"pointer", fontFamily:"inherit", boxShadow:"0 1px 4px rgba(0,0,0,0.06)" }}>
            Save report
          </button>
          <button onClick={() => runReport()}
            style={{ fontSize:11, padding:"6px 14px", borderRadius:20, border:"none", background:B.purple, color:"#fff", cursor:"pointer", fontFamily:"inherit", fontWeight:700 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"260px minmax(0,1fr)", gap:16 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div style={{ display:"flex", gap:4 }}>
            {["build","formulas","saved"].map(p => (
              <Pill key={p} label={p==="build"?"Build":p==="formulas"?"∑ Formulas":"Saved reports"} active={panel===p} onClick={() => setPanel(p)} />
            ))}
          </div>
            ...
            <div style={{ background:B.card,borderRadius:14,padding:16,boxShadow:"0 1px 4px rgba(0,0,0,0.04)" }}>
              ...
              {showSaveForm&&(
                <div style={{ background:B.bg,borderRadius:10,padding:12,marginBottom:12 }}>
                  <Inp val={listName} onChange={setListName} placeholder="Report name…" style={{ marginBottom:8 }} />
                  ...
                  <div style={{ display:"flex",gap:6 }}>
                    <button onClick={()=>{setShowSaveForm(false);setListName("");}} style={{ flex:1,fontSize:11,padding:"6px",borderRadius:8,border:"none",background:B.bg,color:B.gray,cursor:"pointer",fontFamily:"inherit" }}>Cancel</button>
                    <button onClick={saveReport} disabled={!listName||savingList} style={{ flex:1,fontSize:11,padding:"6px",borderRadius:8,border:"none",background:B.purple,color:"#fff",cursor:"pointer",fontFamily:"inherit",fontWeight:700 }}>{savingList?"Saving…":"Save"}</button>
                  </div>
                </div>
              )}
              ...
              {savedList.map(sv=>(
                <div key={sv.id} style={{ display:"flex",alignItems:"center",gap:8,padding:"8px 0",borderBottom:`0.5px solid rgba(0,0,0,0.05)` }}>
                  <div style={{flex:1}}>
                  ...
                  <button onClick={()=>{loadReport(sv);setPanel("build");runReport(sv.object_id);}} style={{ fontSize:11,color:B.purple,background:"none",border:"none",cursor:"pointer",fontWeight:700,fontFamily:"inherit" }}>Load</button>
                  <button onClick={()=>deleteReport(sv.id)} style={{ fontSize:11,color:B.rose,background:"none",border:"none",cursor:"pointer",fontFamily:"inherit" }}>Delete</button>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          {/* Chart */}
          {results&&(
            <Panel title="Visualisation"
              sub={activeChartFilter ? `Filtered to: "${activeChartFilter}" — click again to clear` : "Click a bar, segment or point to filter the table below"}
            >
              <div style={{ display:"flex", gap:6, marginBottom:16 }}>
                ...
                {activeChartFilter&&(
                  <button onClick={()=>setActiveChartFilter(null)}
                    style={{ fontSize:11,padding:"4px 12px",borderRadius:20,border:"none",background:`${B.rose}15`,color:B.rose,cursor:"pointer",fontFamily:"inherit",fontWeight:700,marginLeft:"auto" }}>
                    Clear filter ×
                  </button>
                )}
              </div>
              ...
              {chartType!=="pie"&&(
                <div style={{ display:"flex",gap:12,marginBottom:16 }}>
                  <div style={{flex:1}}><SideLabel>X axis</SideLabel><Sel val={chartX} onChange={setChartX} opts={resultCols.map(k=>({value:k,label:k==="_group"?"Group":k==="_count"?"Count":k}))} /></div>
                  <div style={{flex:1}}><SideLabel>Y axis</SideLabel><Sel val={chartY} onChange={setChartY} opts={resultCols.map(k=>({value:k,label:k==="_group"?"Group":k==="_count"?"Count":k}))} /></div>
                </div>
              )}
              {chartType==="pie"&&(
                <div style={{ display:"flex",gap:12,marginBottom:16 }}>
                  <div style={{flex:1}}><SideLabel>Label</SideLabel><Sel val={chartX} onChange={setChartX} opts={resultCols.map(k=>({value:k,label:k==="_group"?"Group":k}))} /></div>
                  <div style={{flex:1}}><SideLabel>Value</SideLabel><Sel val={chartY} onChange={setChartY} opts={resultCols.map(k=>({value:k,label:k==="_count"?"Count":k}))} /></div>
                </div>
              )}
                ...
                <>
                  {chartType==="bar"&&(
                    <ResponsiveContainer width="100%" height={240}>
                      <BarChart data={chartData} margin={{top:4,right:4,bottom:0,left:-20}} barCategoryGap="30%" onClick={handleChartClick} style={{cursor:"pointer"}}>
                        ...
                        <Tooltip content={<DarkTip/>} cursor={{fill:"rgba(127,119,221,0.05)"}} />
                        <Bar dataKey={chartY} radius={[4,4,0,0]}
                          fill={B.purple}
                          label={{position:"top",fontSize:9,fill:B.gray}}
                          isAnimationActive={false}
                        >
                          {chartData.map((entry,i)=>(
                            <Cell key={i} fill={activeChartFilter&&String(entry[chartX])===activeChartFilter ? B.rose : B.purple} />
                          ))}
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
                  ...
                  {chartType==="area"&&(
                    <ResponsiveContainer width="100%" height={240}>
                      <AreaChart data={chartData} margin={{top:4,right:4,bottom:0,left:-20}} onClick={handleChartClick} style={{cursor:"pointer"}}>
                        <defs>
                  ...
                  {chartType==="pie"&&(
                    <ResponsiveContainer width="100%" height={240}>
                      <PieChart>
                        <Pie data={chartData} dataKey={chartY} nameKey={chartX} cx="50%" cy="50%" innerRadius="55%" outerRadius="78%" paddingAngle={3} onClick={handlePieClick} style={{cursor:"pointer"}}>
                          {chartData.map((entry,i)=>(
                            <Cell key={i} fill={activeChartFilter&&String(entry[chartX])===activeChartFilter ? B.rose : PALETTE[i%PALETTE.length]}
                              opacity={activeChartFilter&&String(entry[chartX])!==activeChartFilter ? 0.4 : 1} />
                          ))}
                        </Pie>
                        <Tooltip content={<DarkTip/>} />
                      </PieChart>
          ...
          {results ? (
            <Panel
              title={activeChartFilter ? `"${activeChartFilter}" — ${(displayedResults||[]).length} rows` : `Results — ${results.length.toLocaleString()} rows`}
              action={
                <div style={{ display:"flex",gap:8,alignItems:"center" }}>
                  {groupBy&&activeChartFilter&&(
                    <button onClick={() => goToFiltered(groupBy, activeChartFilter)}
                      style={{ fontSize:11,color:B.purple,background:`${B.purple}10`,border:"none",cursor:"pointer",fontWeight:700,fontFamily:"inherit",padding:"4px 10px",borderRadius:20 }}>
                      Open in list ↗
                    </button>
                  )}
                    ...
                    <tr>
                      {resultCols.slice(0,8).map(k=>(
                        <th key={k} onClick={()=>{setSortBy(k==="_group"?groupBy:k);setSortDir(d=>d==="asc"?"desc":"asc");}}
                          style={{ textAlign:"left",padding:"8px 10px",fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",color:B.gray,borderBottom:`1px solid rgba(0,0,0,0.05)`,cursor:"pointer",whiteSpace:"nowrap" }}>
                          {k==="_group"?(groupBy||"Group"):k==="_count"?"Count":k}
                          {sortBy===k&&<span style={{marginLeft:4}}>{sortDir==="asc"?"↑":"↓"}</span>}
                        </th>
                      ))}
                      {groupBy&&<th style={{ textAlign:"right",padding:"8px 10px",fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",color:B.gray,borderBottom:`1px solid rgba(0,0,0,0.05)` }}>Action</th>}
                    </tr>
                  </thead>
                  <tbody>
                    {(displayedResults||[]).slice(0,50).map((row,i)=>(
                      <ResultRow key={i} row={row} cols={resultCols.slice(0,8)} groupBy={groupBy}
                        active={activeChartFilter && String(row[chartX]||row._group||"")===activeChartFilter}
                        onFilter={(k,v)=>{ setActiveChartFilter(v); goToFiltered(k,v); }}
                      />
                    ))}
                  </tbody>
                </table>
                {(displayedResults||[]).length>50&&(
                  <div style={{ fontSize:11,color:B.gray,textAlign:"center",marginTop:10 }}>
                    Showing 50 of {(displayedResults||[]).length} rows. Export CSV for the full set.
                  </div>
                )}
              </div>
            </Panel>
            ...
            <div style={{ background:B.card,borderRadius:14,padding:48,textAlign:"center",boxShadow:"0 1px 4px rgba(0,0,0,0.04)" }}>
              ...
              <div style={{ fontSize:13,color:B.gray,marginBottom:20,maxWidth:280,margin:"0 auto 20px" }}>
                Select a data source, choose columns, add filters, then hit Run report.
              </div>
...
function ResultRow({ row, cols, groupBy, active, onFilter }) {
  ...
  return (
    <tr onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ background: active ? "rgba(127,119,221,0.06)" : hov ? "#F8F7FF" : "transparent", transition:"background 0.1s" }}>
      {cols.map(k=>(
        <td key={k} style={{ padding:"9px 10px", color: k==="_count" ? "#7F77DD" : "#374151", fontWeight: k==="_count" ? 700 : 400, cursor: groupBy&&k==="_group" ? "pointer" : "default" }}
          onClick={() => { if(groupBy&&k==="_group"&&row[k]) onFilter(groupBy,row[k]); }}>
          {k==="_count"
            ? <span style={{ color:"#7F77DD",fontWeight:700 }}>{row[k]}</span>
      ...
      {groupBy&&(
        <td style={{ padding:"9px 10px",textAlign:"right" }}>
          {row._group&&(
            <button onClick={()=>onFilter(groupBy,row._group)}
              style={{ fontSize:10,color:"#7F77DD",background:`rgba(127,119,221,0.1)`,border:"none",cursor:"pointer",fontFamily:"inherit",fontWeight:700,padding:"3px 8px",borderRadius:20,opacity:hov?1:0,transition:"opacity 0.1s" }}>
              View list ↗
            </button>
          )}
        </td>
