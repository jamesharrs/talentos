...
const PATHS = {
  ...
  chart:"M18 20V10M12 20V4M6 20v-6", arrow:"M5 12h14M12 5l7 7-7 7",
...
function TemplateEditor({ template, onSave, onClose }) {
  ...
  const sendTest = async () => {
    if(!testAddr||!template?.id) return;
    setTestResult(null);
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:C.surface,borderRadius:16,width:"min(900px,96vw)",height:"90vh",display:"flex",flexDirection:"column",overflow:"hidden",boxShadow:"0 24px 80px rgba(0,0,0,.2)"}}>
        <div style={{padding:"16px 24px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          <div style={{fontSize:16,fontWeight:700}}>{isNew?"New Template":"Edit Template"}</div>
        ...
        <div style={{display:"flex",gap:0,borderBottom:`1px solid ${C.border}`,flexShrink:0}}>
          {[{id:'edit',label:'Edit'},{id:'preview',label:'Preview'},{id:'merge',label:'Merge Tags'}].map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"10px 18px",border:"none",background:"transparent",fontSize:13,fontWeight:tab===t.id?700:500,color:tab===t.id?C.accent:C.text3,borderBottom:tab===t.id?`2px solid ${C.accent}`:"2px solid transparent",cursor:"pointer",fontFamily:F,marginBottom:-1}}>{t.label}</button>
          ))}
        </div>
        <div style={{flex:1,overflowY:"auto",padding:"20px 24px"}}>
          {tab==='edit' && <>
            ...
            <Inp label="Email Body (HTML)" value={form.body_html} onChange={v=>set('body_html',v)} multiline rows={14} placeholder="<p>Hi {{admin_first_name}},</p>..."/>
            <Inp label="Plain Text Fallback" value={form.body_text} onChange={v=>set('body_text',v)} multiline rows={3}/>
            {!isNew && (
              <div style={{marginTop:8,padding:"14px 16px",background:C.bg,borderRadius:10,border:`1px solid ${C.border}`}}>
                <div style={{fontSize:12,fontWeight:700,color:C.text2,marginBottom:8}}>Test send (uses sample data)</div>
                <div style={{display:"flex",gap:8}}>
                  ...
                  <Btn icon="send" disabled={!testAddr} onClick={sendTest}>Send test</Btn>
                </div>
          ...
          {tab==='preview' && (
            <div>
              <div style={{marginBottom:12,padding:"10px 14px",background:C.bg,borderRadius:8,fontSize:12,color:C.text3}}>Preview with sample data: <strong>James @ Acme Corp</strong></div>
              <div style={{border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden"}}>
                ...
                <iframe srcDoc={preview} style={{width:"100%",height:500,border:"none"}} title="preview"/>
              </div>
            </div>
          )}
          {tab==='merge' && (
            <div>
              <div style={{marginBottom:16,fontSize:13,color:C.text2,lineHeight:1.6}}>Copy these into your subject or body — replaced with real values on send.</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10}}>
                {MERGE_TAGS.map(tag=>(
                  <div key={tag} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px",background:C.bg,borderRadius:8,border:`1px solid ${C.border}`}}>
                    ...
                    <button onClick={()=>navigator.clipboard.writeText(tag)} style={{border:"none",background:"none",cursor:"pointer",color:C.text3}}><Ic n="copy" s={13}/></button>
                  </div>
                ))}
              </div>
            </div>
...
function SequenceBuilder({ sequence, templates, milestones, goals, onSave, onClose }) {
  ...
  useEffect(()=>{ if(sequence?.id) API(`/sequences/${sequence.id}/steps`).then(setSteps); },[sequence?.id]);

  const save = async () => {
    if(!form.name||!form.trigger) return alert('Name and trigger required');
    setSaving(true);
    const result = isNew ? await API('/sequences',{method:'POST',body:JSON.stringify(form)}) : await API(`/sequences/${sequence.id}`,{method:'PATCH',body:JSON.stringify(form)});
    setSaving(false);
    if(result.error) return alert(result.error);
    onSave(result);
  };
  ...
  const addStep = async () => {
    ...
    if(!sequence?.id) return alert('Save the sequence first, then add steps');
    const result = await API(`/sequences/${sequence.id}/steps`,{method:'POST',body:JSON.stringify(newStep)});
    if(result.error) return alert(result.error);
    setSteps(s=>[...s,result]);
  ...
  const removeStep = async (stepId) => {
    await API(`/sequences/${sequence.id}/steps/${stepId}`,{method:'DELETE'});
    setSteps(s=>s.filter(x=>x.id!==stepId));
  };

  const COND_OPTS = [
    {value:'goal_not_met',label:'Send if goal not yet met'},{value:'always',label:'Always send'},
    {value:'email_opened',label:'Send if previous email was opened'},{value:'email_not_opened',label:'Send if previous email was NOT opened'},
  ];
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{background:C.surface,borderRadius:16,width:"min(760px,96vw)",maxHeight:"90vh",display:"flex",flexDirection:"column",overflow:"hidden",boxShadow:"0 24px 80px rgba(0,0,0,.2)"}}>
        <div style={{padding:"16px 24px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          <div style={{fontSize:16,fontWeight:700}}>{isNew?"New Sequence":"Edit Sequence"}</div>
          <div style={{display:"flex",gap:8}}>
            <Btn v="secondary" icon="x" onClick={onClose}>Cancel</Btn>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"20px 24px"}}>
          ...
          <Inp label="Description" value={form.description} onChange={v=>set('description',v)} placeholder="Optional"/>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <Sel label="Trigger" value={form.trigger} onChange={v=>set('trigger',v)} options={milestones.map(m=>({value:m.id,label:m.label}))}/>
            <Sel label="Goal (stops sequence)" value={form.goal} onChange={v=>set('goal',v)} options={goals.map(g=>({value:g.id,label:g.label}))}/>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20,padding:"12px 16px",background:form.active?C.greenL:C.bg,borderRadius:10,border:`1px solid ${form.active?C.green+"40":C.border}`}}>
            <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13,fontWeight:600,color:form.active?C.green:C.text3}}>
              ...
              {form.active?"Active — auto-enrols clients":"Inactive — paused"}
            </label>
          </div>
          {!isNew && <>
            ...
            {steps.length===0 && <div style={{textAlign:"center",padding:"32px 0",color:C.text3,fontSize:13,background:C.bg,borderRadius:10,border:`1px dashed ${C.border}`}}>No steps yet — add one above</div>}
            {steps.map((step,i)=>{
              const tmpl=templates.find(t=>t.id===step.template_id);
              const cond=COND_OPTS.find(o=>o.value===step.condition);
              return (
                <div key={step.id} style={{display:"flex",gap:12,marginBottom:10,alignItems:"flex-start"}}>
                  <div style={{width:28,height:28,borderRadius:"50%",background:C.accent,color:"white",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:800,flexShrink:0,marginTop:12}}>{i+1}</div>
                  <div style={{flex:1,padding:"12px 16px",background:C.bg,borderRadius:10,border:`1px solid ${C.border}`}}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                      <div style={{fontSize:13,fontWeight:700}}>{tmpl?.name||"Unknown template"}</div>
                      <button onClick={()=>removeStep(step.id)} style={{border:"none",background:"none",cursor:"pointer",color:C.text3}}><Ic n="trash" s={13}/></button>
                    </div>
                    <div style={{marginTop:4,display:"flex",flexWrap:"wrap",gap:6}}>
                      <Badge color={C.text3}>{(step.delay_days||0)+(step.delay_hours||0)===0?"Immediately":`${step.delay_days||0}d ${step.delay_hours||0}h delay`}</Badge>
            ...
            {showAddStep && (
              <div style={{marginTop:12,padding:"16px",background:C.accentL,borderRadius:12,border:`1.5px solid ${C.accent}40`}}>
                <div style={{fontSize:13,fontWeight:700,marginBottom:12}}>Add Email Step</div>
                <Sel label="Template" value={newStep.template_id} onChange={v=>setNewStep(s=>({...s,template_id:v}))} options={[{value:'',label:'Select a template…'},...templates.map(t=>({value:t.id,label:t.name}))]}/>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                ...
                <Sel label="Condition" value={newStep.condition} onChange={v=>setNewStep(s=>({...s,condition:v}))} options={COND_OPTS}/>
          ...
          {isNew && <div style={{padding:"12px 16px",background:C.amberL,borderRadius:8,fontSize:12,color:C.amber}}>Save first, then add email steps.</div>}
        </div>
      </div>
    </div>
  );
...
export default function EmailSequencer() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [tmpl,seqs,log,meta] = await Promise.all([API('/templates'),API('/sequences'),API('/send-log'),API('/milestones')]);
  ...
  const loadStats = async (seqId) => {
    setStatsFor(seqId); setStats(null);
        ...
        {[
          ...
          <div key={card.label} style={{background:C.surface,borderRadius:12,padding:"16px 18px",border:`1px solid ${C.border}`}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
              <div style={{width:32,height:32,borderRadius:9,background:card.color+"18",display:"flex",alignItems:"center",justifyContent:"center"}}><Ic n={card.icon} s={15} c={card.color}/></div>
            ...
            <div key={seq.id} style={{background:C.surface,borderRadius:12,border:`1px solid ${C.border}`,padding:"16px 20px",marginBottom:10}}>
              <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between"}}>
                <div style={{flex:1,minWidth:0}}>
                  ...
                  {seq.description && <div style={{fontSize:12,color:C.text3,marginBottom:6}}>{seq.description}</div>}
                ...
                <div style={{display:"flex",gap:4,flexShrink:0}}>
                  <button onClick={()=>loadStats(seq.id)} title="Stats" style={{border:"none",background:"none",cursor:"pointer",color:C.text3,padding:6}}><Ic n="chart" s={14}/></button>
                  <button onClick={()=>toggleActive(seq)} title={seq.active?"Pause":"Activate"} style={{border:"none",background:"none",cursor:"pointer",color:seq.active?C.amber:C.green,padding:6}}><Ic n="zap" s={14}/></button>
              ...
              {statsFor===seq.id && stats && (
                <div style={{marginTop:14,paddingTop:14,borderTop:`1px solid ${C.border}`}}>
                  <div style={{display:"flex",gap:16,flexWrap:"wrap",marginBottom:10}}>
                    {[{l:"Enrolled",v:stats.total_enrolled,c:C.accent},{l:"Active",v:stats.active,c:C.green},{l:"Completed",v:stats.completed,c:"#0891b2"},{l:"Goal met",v:stats.goal_met,c:C.green},{l:"Sent",v:stats.total_sent,c:C.text2},{l:"Opened",v:stats.total_opened,c:C.amber}].map(s=>(
                  ...
                  {stats.step_stats?.map((s,i)=>(
                    <div key={s.step_id} style={{display:"flex",alignItems:"center",gap:10,marginBottom:5,fontSize:12}}>
                      ...
                      <div style={{flex:1,height:6,borderRadius:99,background:"#f3f4f6",overflow:"hidden"}}>
                        <div style={{height:"100%",background:C.green,width:`${s.open_rate}%`,transition:"width .3s"}}/>
                      </div>
                      <span style={{color:C.text3,minWidth:90,textAlign:"right"}}>{s.sent} sent · {s.open_rate}% open</span>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14}}>
          ...
          {templates.map(t=>(
            <div key={t.id} style={{background:C.surface,borderRadius:12,border:`1px solid ${C.border}`,padding:"16px 18px"}}>
              ...
              <div style={{fontSize:12,color:C.text3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginBottom:4}}>{t.subject}</div>
