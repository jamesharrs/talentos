...
const C = {
  ...
  text1:"#111827", text2:"#4B5563", text3:"#9CA3AF", border:"#E5E7EB", bg:"#F9FAFB",
...
const PATHS = {
  ...
  check:"M20 6L9 17l-5-5", zap:"M13 2L3 14h9l-1 10 10-12h-9l1-10z",
  barChart:"M12 20V10M18 20V4M6 20v-4", target:"M12 22a10 10 0 1 0 0-20 10 10 0 0 0 0 20zM12 18a6 6 0 1 0 0-12 6 6 0 0 0 0 12zM12 14a2 2 0 1 0 0-4 2 2 0 0 0 0 4z",
  refresh:"M23 4v6h-6M1 20v-6h6M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15",
  info:"M12 22a10 10 0 1 0 0-20 10 10 0 0 0 0 20zM12 16v-4M12 8h.01",
  dollar:"M12 2v20M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6",
};
const Ic = ({ n, s=16, c="currentColor" }) => (
  <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d={PATHS[n]||PATHS.info}/></svg>
);
...
const StatCard = ({ icon, iconColor, iconBg, label, value, sub }) => (
  <div style={{ background:"white", borderRadius:12, border:`1px solid ${C.border}`, padding:"10px 12px", display:"flex", alignItems:"center", gap:12, flex:1 }}>
    ...
    <div><div style={{ fontSize:16, fontWeight:800, color:C.text1, lineHeight:1.1 }}>{value}</div>
      <div style={{ fontSize:11, color:C.text3, fontWeight:500, marginTop:1 }}>{label}</div>
      {sub && <div style={{ fontSize:10, color:C.text3, marginTop:2 }}>{sub}</div>}</div>
  </div>
);

const RiskBadge = ({ level }) => {
  const cfg = { high:{ bg:C.redBg, color:C.red }, medium:{ bg:C.amberBg, color:C.amber }, low:{ bg:C.greenBg, color:C.green } };
  const cl = cfg[level]||cfg.low;
  return <span style={{ fontSize:10, fontWeight:700, padding:"2px 8px", borderRadius:99, background:cl.bg, color:cl.color, textTransform:"uppercase" }}>{level}</span>;
};

const HealthDot = ({ health }) => {
  const colors = { fast:C.green, normal:C.blue, slow:C.amber, bottleneck:C.red };
  return <span style={{ display:"inline-block", width:8, height:8, borderRadius:"50%", background:colors[health]||C.text3 }} title={health}/>;
};

const ProgressBar = ({ value, max, color, height=6 }) => (
  <div style={{ height, borderRadius:height, background:"#F3F4F6", width:"100%", overflow:"hidden" }}>
    <div style={{ height:"100%", borderRadius:height, background:color, width:`${max>0?Math.min(100,(value/max)*100):0}%`, transition:"width 0.4s ease" }}/>
  </div>
);

const OnTrackBadge = ({ status }) => {
  const cfg = { ahead:{bg:C.greenBg,color:C.green,label:"Ahead of schedule"}, on_track:{bg:C.blueBg,color:C.blue,label:"On track"}, at_risk:{bg:C.amberBg,color:C.amber,label:"At risk"}, overdue:{bg:C.redBg,color:C.red,label:"Overdue"} };
  const cl = cfg[status]||{bg:C.bg,color:C.text3,label:"No data"};
  return <div style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"4px 10px", borderRadius:8, background:cl.bg, fontSize:12, fontWeight:700, color:cl.color }}>
    <Ic n={status==="ahead"||status==="on_track"?"check":"alert"} s={13} c={cl.color}/>{cl.label}
  </div>;
};
...
export default function InsightsPanel({ record, environment }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/analytics/job-insights?job_id=${record.id}&environment_id=${environment.id}`);
  ...
  if (error) return <div style={{ padding:24, textAlign:"center" }}>
    <div style={{ color:C.red, fontSize:13, marginBottom:8 }}>Failed to load insights</div>
    <button onClick={load} style={{ background:"none", border:`1px solid ${C.border}`, borderRadius:8, padding:"6px 14px", fontSize:12, cursor:"pointer", color:C.text2, fontFamily:F }}>Retry</button>
  </div>;
  if (!data) return null;

  const tabs = [
    { id:"overview", label:"Overview" }, { id:"pipeline", label:"Pipeline" },
    { id:"risk", label:`Risk (${data.candidate_risk?.filter(c=>c.risk_level!=="low").length||0})` },
    ...
    <div style={{ fontFamily:F }}>
      <div style={{ display:"flex", gap:2, marginBottom:16, background:"#F3F4F6", borderRadius:8, padding:3 }}>
        {tabs.map(t => <button key={t.id} onClick={()=>setActiveTab(t.id)} style={{
          ...
          background:activeTab===t.id?"white":"transparent", color:activeTab===t.id?C.text1:C.text3,
          fontSize:12, fontWeight:activeTab===t.id?700:500, fontFamily:F,
      ...
      {activeTab==="overview" && <div>
        ...
        {data.time_to_fill?.on_track && <div style={{ marginBottom:14 }}>
          ...
          {data.time_to_fill.estimated_days&&data.time_to_fill.days_open!=null&&<div style={{ marginTop:8 }}>
        ...
        {data.candidate_risk?.length>0 && <div style={{ background:"white", borderRadius:10, border:`1px solid ${C.border}`, padding:"12px 14px" }}>
          ...
          <div style={{ display:"flex", gap:12 }}>
            {["high","medium","low"].map(level => { const count=data.candidate_risk.filter(c=>c.risk_level===level).length; const colors={high:C.red,medium:C.amber,low:C.green};
              return <div key={level} style={{ display:"flex", alignItems:"center", gap:6 }}><div style={{ width:10, height:10, borderRadius:"50%", background:colors[level] }}/><span style={{ fontSize:12, color:C.text2 }}>{count} {level}</span></div>;
            })}
          </div>
        </div>}
      </div>}
        ...
        {data.pipeline?.stages?.length>0 ? <>
          {data.pipeline.stages.map((stage,i) => <div key={stage.name} style={{ marginBottom:2 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 12px", borderRadius:8,
              background:stage.health==="bottleneck"?C.redBg:stage.health==="slow"?C.amberBg:stage.health==="fast"?C.greenBg:"white",
              border:`1px solid ${stage.health==="bottleneck"?"#FCA5A5":stage.health==="slow"?"#FCD34D":C.border}` }}>
              <HealthDot health={stage.health}/>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:13, fontWeight:600, color:C.text1 }}>{stage.name}</div>
                <div style={{ fontSize:11, color:C.text3 }}>{stage.count} candidate{stage.count!==1?"s":""}{stage.avg_days!=null&&` · avg ${stage.avg_days}d`}{stage.env_avg_days!=null&&` (norm: ${stage.env_avg_days}d)`}</div>
              </div>
      ...
      {activeTab==="risk" && <div>
        {data.candidate_risk?.length>0 ? data.candidate_risk.map((c,i)=><div key={i} style={{
          display:"flex", alignItems:"flex-start", gap:10, padding:"10px 12px", marginBottom:6, borderRadius:8,
          background:c.risk_level==="high"?C.redBg:c.risk_level==="medium"?C.amberBg:"white",
          border:`1px solid ${c.risk_level==="high"?"#FCA5A5":c.risk_level==="medium"?"#FCD34D":C.border}` }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={{ fontSize:11, color:C.text3, marginBottom:4 }}>Stage: {c.stage}{c.days_in_stage!=null&&` · ${c.days_in_stage}d in stage`}{c.days_since_contact!=null&&` · Last contact ${c.days_since_contact}d ago`}</div>
            {c.factors.length>0&&<div style={{ display:"flex", flexWrap:"wrap", gap:4 }}>
              {c.factors.map((f,fi)=><span key={fi} style={{ fontSize:10, padding:"2px 6px", borderRadius:4, background:c.risk_level==="high"?"#FEE2E2":"#FEF3C7", color:c.risk_level==="high"?C.red:C.amber }}>{f}</span>)}
            </div>}
          </div>
      ...
      {activeTab==="sources" && <div>
        {data.source_effectiveness?.length>0 ? data.source_effectiveness.map((s,i)=><div key={i} style={{ padding:"10px 12px", marginBottom:6, borderRadius:8, background:"white", border:`1px solid ${C.border}` }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:6 }}>
            <span style={{ fontSize:13, fontWeight:600, color:C.text1 }}>{s.source}</span>
            <span style={{ fontSize:12, color:C.text3 }}>{s.total} candidates</span>
          </div>
