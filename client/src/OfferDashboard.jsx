...
export default function OfferDashboard({ environment, session, onNavigate }) {
  ...
  if (loading) return (
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:24, flexWrap:"wrap", padding:"14px 18px", background:"white", borderRadius:14, border:"1px solid #f0f0f0" }}>
        ...
        <button onClick={()=>{window.dispatchEvent(new CustomEvent('talentos:nav',{detail:'offers'}))}} style={{ display:"flex", alignItems:"center", gap:6, padding:"8px 14px", borderRadius:9, border:`1.5px solid #f59f0030`, background:`#f59f0008`, cursor:"pointer", fontSize:13, fontWeight:600, color:"#f59f00", fontFamily:"inherit", whiteSpace:"nowrap" }}>Pending Approval</button>
