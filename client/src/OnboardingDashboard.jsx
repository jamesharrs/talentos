...
const PATHS = {
  ...
  check:"M20 6L9 17l-5-5", file:"M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8zM14 2v6h6",
...
const Ic = ({ n, s=16, c="currentColor" }) => (
  <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d={PATHS[n]||""} /></svg>
);

const QuickLink = ({ icon, label, color, onClick, badge, comingSoon }) => (
  <button onClick={onClick} style={{ display:"flex", alignItems:"center", gap:8, padding:"9px 15px", borderRadius:10, border:`1.5px solid ${comingSoon?"#e5e7eb":color+"30"}`, background:comingSoon?"#f9fafb":`${color}08`, cursor:"pointer", fontFamily:F, transition:"all .15s", whiteSpace:"nowrap" }}
    onMouseEnter={e=>{ if(!comingSoon){e.currentTarget.style.background=`${color}18`; e.currentTarget.style.borderColor=`${color}60`; }}}
    onMouseLeave={e=>{ if(!comingSoon){e.currentTarget.style.background=`${color}08`; e.currentTarget.style.borderColor=`${color}30`; }}}>
    <Ic n={icon} s={14} c={comingSoon?C.text3:color}/>
...
const StatCard = ({ icon, value, label, sub, color=C.accent, alert }) => (
  <div style={{ background:C.card, borderRadius:14, padding:"18px 20px", border:`1.5px solid ${alert?C.red+"40":C.border}`, boxShadow:"0 1px 4px rgba(0,0,0,0.04)", position:"relative" }}>
    ...
    <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:10 }}>
      ...
      <div><div style={{ fontSize:28, fontWeight:800, color:C.text1, lineHeight:1 }}>{value??'—'}</div><div style={{ fontSize:12, fontWeight:600, color:C.text2, marginTop:3 }}>{label}</div></div>
    </div>
...
const Card = ({ children, title, titleRight, style={} }) => (
  <div style={{ background:C.card, borderRadius:14, border:`1px solid ${C.border}`, boxShadow:"0 1px 4px rgba(0,0,0,0.04)", padding:"20px 22px", ...style }}>
    {(title||titleRight)&&<div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>{title&&<div style={{ fontSize:14, fontWeight:700, color:C.text1 }}>{title}</div>}{titleRight}</div>}
    {children}
  </div>
);

const PlaceholderTile = ({ icon, title, description, color }) => (
  <div style={{ background:"#fafafa", borderRadius:12, border:`1.5px dashed ${C.border}`, padding:"20px", textAlign:"center" }}>
    <div style={{ width:40, height:40, borderRadius:10, background:`${color}15`, display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 10px" }}><Ic n={icon} s={20} c={color}/></div>
    ...
    <div style={{ marginTop:8, display:"inline-block", fontSize:10, fontWeight:700, color, background:`${color}15`, padding:"2px 8px", borderRadius:99 }}>Coming Soon</div>
  </div>
);
...
export default function OnboardingDashboard({ environment, onNavigate }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const onboarding = people.filter(p=>{ const s=(p.data?.status||'').toLowerCase(); return s.includes('hired')||s.includes('onboard')||s.includes('starting'); });
      const probationDue = people.filter(p=>{ const d=daysUntil(p.data?.probation_end||p.data?.probation_review_date); return d!=null&&d>=0&&d<=14; });

      const monthlyMap = {};
      for(let i=5;i>=0;i--){ const d=new Date(); d.setMonth(d.getMonth()-i); const key=d.toLocaleString('default',{month:'short'}); monthlyMap[key]=0; }
      acceptedOffers.forEach(o=>{ const sd=o.data?.start_date||o.start_date; if(sd){ const key=new Date(sd).toLocaleString('default',{month:'short'}); if(monthlyMap[key]!==undefined) monthlyMap[key]++; } });
  ...
  return (
    <div style={{ fontFamily:F, color:C.text1, padding:"28px 32px 80px", background:"#F8F7FF", minHeight:"100%" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
        <div>
          <div style={{ fontSize:22, fontWeight:800, color:C.text1, letterSpacing:"-0.02em" }}>Onboarding Dashboard</div>
          <div style={{ fontSize:12, color:C.text3, marginTop:3 }}>Accepted offers, document status &amp; day-one readiness</div>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
          {[{label:"Overview",nav:"overview",color:"#6B7280"},{label:"Screening",nav:"screening",color:"#7F77DD"},{label:"Interviews",nav:"interviews",color:"#1D9E75"},{label:"Offers",nav:"offers",color:"#D4537E"},{label:"Onboarding",nav:"onboarding",color:"#EF9F27",current:true}]
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:24, flexWrap:"wrap", padding:"14px 18px", background:C.card, borderRadius:14, border:`1px solid ${C.border}` }}>
        ...
        <QuickLink icon="file"      label="Docs Pending"        color={C.amber}  onClick={()=>nav("people")} badge={docsNeeded.length}/>
        ...
        <QuickLink icon="mail"      label="Welcome Email"       color={C.green}  comingSoon/>
      </div>
      ...
      {phase==="pre"&&(
        <>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16, marginBottom:24 }}>
            ...
            <StatCard icon="calendar" value={startingThisMonth.length} label="Starting This Month" color={C.teal}  sub="within 30 days"/>
            ...
            <StatCard icon="briefcase" value={onboarding.length}       label="In Onboarding"       color={C.purple} sub="active"/>
          </div>
                ...
                {[
                  ...
                  <div key={i} style={{ display:"flex", alignItems:"center", gap:10 }}>
                    <div style={{ width:20, height:20, borderRadius:"50%", background:item.soon?"#f3f4f6":item.done?C.green+"20":C.amber+"20", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                      <Ic n={item.soon?"clock":item.done?"check":"clock"} s={11} c={item.soon?C.text3:item.done?C.green:C.amber}/>
                    </div>
                    <div style={{ flex:1 }}><span style={{ fontSize:12, color:item.soon?C.text3:C.text1, fontWeight:600 }}>{item.label}</span></div>
          ...
          <Card title="Upcoming Starters">
            {acceptedOffers.length===0 ? <div style={{ textAlign:"center", padding:"32px 0", color:C.text3, fontSize:13 }}>No accepted offers yet</div> : (
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                <thead><tr>{["Candidate","Role","Start Date","Days Until","Docs Status"].map(h=><th key={h} style={{ textAlign:"left", fontSize:11, fontWeight:700, color:C.text3, padding:"6px 12px", textTransform:"uppercase", letterSpacing:"0.05em", borderBottom:`1px solid ${C.border}` }}>{h}</th>)}</tr></thead>
                <tbody>{acceptedOffers.slice(0,8).map((o,i)=>{ const d=daysUntil(o.data?.start_date||o.start_date); const ds=o.data?.docs_status||'pending'; return (
                  <tr key={o.id} style={{ background:i%2===0?"transparent":"#fafafa" }}>
                    <td style={{ padding:"10px 12px", fontSize:13, fontWeight:600 }}>{o.candidate_name||`Candidate ${i+1}`}</td>
                    ...
                    <td style={{ padding:"10px 12px" }}>{d!=null?<span style={{ fontSize:12, fontWeight:700, color:d<=7?C.red:d<=14?C.amber:C.green }}>{d===0?"Today":d<0?"Started":d+" days"}</span>:<span style={{ color:C.text3 }}>—</span>}</td>
                    <td style={{ padding:"10px 12px" }}><span style={{ fontSize:11, padding:"3px 9px", borderRadius:99, fontWeight:600, background:ds==='complete'?C.green+"15":C.amber+"15", color:ds==='complete'?C.green:C.amber }}>{ds}</span></td>
                  </tr>
                );})}
                </tbody>
              </table>
      ...
      {phase==="post"&&(
        <>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:20, marginBottom:24 }}>
            <PlaceholderTile icon="clipboard" title="Task Management"       description="Assign and track onboarding tasks — induction, training, system access."      color={C.purple}/>
            <PlaceholderTile icon="shield"    title="Right to Work"         description="Track RTW document status, expiry dates and re-verification reminders."       color={C.blue}/>
            <PlaceholderTile icon="star"      title="Probation Milestones"  description="Set review dates, record feedback, trigger extensions or confirmations."      color={C.amber}/>
            <PlaceholderTile icon="briefcase" title="Equipment & Access"    description="Manage laptop, access cards, software licences — requested before day 1."     color={C.teal}/>
            <PlaceholderTile icon="mail"      title="Welcome Communications" description="Automated welcome emails, day-1 schedule, team intro packs."                 color={C.green}/>
            <PlaceholderTile icon="file"      title="Document Vault"        description="Secure storage of contracts, ID docs, certifications and signed policies."    color={C.text2}/>
          </div>
          <Card title="Currently Onboarding">
            {onboarding.length===0 ? <div style={{ textAlign:"center", padding:"32px 0", color:C.text3, fontSize:13 }}>No one in onboarding status.<br/><span style={{ fontSize:12 }}>People with status "Hired" or "Onboarding" appear here.</span></div> : (
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                ...
                <tbody>{onboarding.slice(0,10).map((p,i)=>{ const name=[p.data?.first_name,p.data?.last_name].filter(Boolean).join(' ')||p.data?.email||'Unknown'; const pd=daysUntil(p.data?.probation_end||p.data?.probation_review_date); return (
                  <tr key={p.id} style={{ background:i%2===0?"transparent":"#fafafa", cursor:"pointer" }} onClick={()=>window.dispatchEvent(new CustomEvent("talentos:openRecord",{detail:{recordId:p.id,objectId:p.object_id}}))}>
                    ...
                    <td style={{ padding:"10px 12px" }}><span style={{ fontSize:11, padding:"3px 9px", borderRadius:99, fontWeight:600, background:`${C.purple}15`, color:C.purple }}>{p.data?.status||'Onboarding'}</span></td>
                    <td style={{ padding:"10px 12px", fontSize:12, color:pd!=null&&pd<=14?C.red:C.text2 }}>{p.data?.probation_end||p.data?.probation_review_date?`${fmtDate(p.data?.probation_end||p.data?.probation_review_date)}${pd!=null&&pd<=14?` (${pd}d)`:""}`:"—"}</td>
                  </tr>
                );})}
                </tbody>
              </table>
