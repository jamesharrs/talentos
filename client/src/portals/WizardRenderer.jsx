...
export default function WizardRenderer({ portal, wizard, job, api, onBack, onSuccess }) {
  ...
  useEffect(() => {
    ...
    api.get(`/screening/job/${job.id}`).then(data => {
      ...
      // Map screening rules → question format the block expects
      ...
      // Knockouts first, then required, then preferred
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // CV / Resume — upload separately so it lands as a typed attachment on the profile
