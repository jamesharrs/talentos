            ...
            <div style={{marginTop:10}}>
              ...
              {(otpState==='sent'||otpState==='error')&&(
                <div style={{padding:'14px',background:'#F0F9FF',border:'1px solid #BAE6FD',borderRadius:10}}>
                  ...
                  <button onClick={()=>{ setOtpCode(''); /* re-send */ onEmailBlur(); }}
...
export default function WizardRenderer({ portal, wizard, job, api, onBack, onSuccess }) {
  ...
  const [otpState, setOtpState]        = useState(null); // null | 'sending' | 'sent' | 'verified' | 'error'
  const [otpCode, setOtpCode]          = useState('');
  const [otpError, setOtpError]        = useState('');
  const [otpSimCode, setOtpSimCode]    = useState(''); // shown in dev/simulation mode
